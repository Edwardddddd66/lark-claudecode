...
在 Lark / 飞书里直接和你本机的 Claude Code 对话。
...
使用 [uv](https://github.com/astral-sh/uv) 管理环境（也可用标准库 `venv`）：

```bash
git clone https://github.com/Edwardddddd66/lark-claudecode.git
cd lark-claudecode
...
> 不用 uv 的话：`python3 -m venv .venv && .venv/bin/pip install -r requirements.txt`。
> 运行时 `start.sh` 直接调用 `.venv/bin/python3`，不依赖 uv。
...
| `/provider` | 查看/切换后端（按钮选择） |
| `/provider mimo` | 切到小米 MiMo API（自动开新 session） |
| `/provider anthropic` | 切回 Claude Max/Pro 订阅 |
...
| `DEFAULT_PROVIDER` | 否 | `anthropic` | 默认后端：`anthropic`（订阅）/ `mimo` |
| `MIMO_BASE_URL` | 否 | MiMo 官方端点 | MiMo Anthropic 兼容端点 |
| `MIMO_API_KEY` | 否 | - | MiMo API Key（切到 mimo 后端时必填） |
| `MIMO_MODEL` | 否 | `mimo-v2.5-pro` | MiMo 模型名 |

## 多后端（Provider）

默认走 Claude Max/Pro 订阅（不需要 API Key）。如果订阅额度耗尽，可切到小米 MiMo API：

1. 在 `.env` 填入 `MIMO_API_KEY`（其余 MiMo 变量有默认值）
2. 在 Lark 里发 `/provider mimo` 切换，`/provider anthropic` 切回

两端的对话历史格式不兼容（MiMo 返回的 thinking 块 signature 为空），所以**切换后端会自动开启新 session**，旧会话仍可用 `/resume` 找回。切到 MiMo 时模型固定为 `MIMO_MODEL`，订阅模式下才使用 `/model` 设置的模型。
